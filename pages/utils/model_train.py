...
def get_forecast(original_price, differencing_order):
    ...
    # FIX: Create an index with exactly 30 business day periods
    forecast_index = pd.date_range(start=start_date, periods=30, freq='B')
    
    forecast_df = pd.DataFrame(predictions, index = forecast_index, columns = ['Close'])
    return forecast_df
# FIXED
def inverse_scaling(scaler, scaled_data):
    # Reshape the 1D data to 2D for the scaler
    scaled_data_reshaped = np.array(scaled_data).reshape(-1, 1)
    
    # Inverse transform the data
    inversed_data = scaler.inverse_transform(scaled_data_reshaped)
    
    # Return the data as a 1D array (which is what you want for the DataFrame)
    return inversed_data.flatten()